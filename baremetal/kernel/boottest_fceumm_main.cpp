...
static void WriteUnsigned(CScreenDevice &screen, const char *prefix, unsigned value)
{
	char buffer[64];
	char digits[16];
	unsigned used = 0;
	...
	while (prefix && *prefix && pos < sizeof(buffer) - 1)
	{
		buffer[pos++] = *prefix++;
	}
	...
	while (used && pos < sizeof(buffer) - 1)
	{
		buffer[pos++] = digits[--used];
	}

	buffer[pos] = 0;
	WriteLine(screen, buffer);
}
...
int main(void)
{
	...
	DrawBands(screen, 0);
	WriteLine(screen, "Circle + linked FCEUmm boot test: screen initialized");
	...
	logger.Write("fceutest", LogNotice, "logger initialized");
	...
	WriteLine(screen, "calling retro_api_version...");
	const unsigned apiVersion = retro_api_version();
	WriteUnsigned(screen, "retro_api_version=", apiVersion);
	WriteUnsigned(screen, "RETRO_API_VERSION=", RETRO_API_VERSION);

	for (unsigned phase = 0;; phase++)
	{
		DrawBands(screen, phase);
		WriteLine(screen, "FCEUmm link test alive");
